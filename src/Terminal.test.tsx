...
describe("Terminal - rendering", () => {
  ...
  it("renders initialHistory entries", () => {
    ...
    render(
      <Terminal commands={noopCommands} initialHistory={initialHistory} />,
    );
  ...
  it("applies preset theme CSS variables to the root element", () => {
    const { container } = render(
      <Terminal preset="matrix" commands={noopCommands} />,
    );
    const root = container.firstElementChild as HTMLElement;
    expect(root.style.getPropertyValue("--terminal-bg")).toBe(
      TERMINAL_PRESETS.matrix.bg,
    );
    expect(root.style.getPropertyValue("--terminal-text")).toBe(
      TERMINAL_PRESETS.matrix.text,
    );
  });
...
describe("Terminal - command handling", () => {
  it("echoes submitted commands into history with the prompt prefix and clears the field", async () => {
    ...
    expect(
      await screen.findByText("user@t:~$ hello world"),
    ).toBeInTheDocument();
  ...
  it("reports unknown commands through commandNotFoundFormatter", async () => {
    ...
    expect(
      await screen.findByText('[ERR] "frobnicate" is unknown'),
    ).toBeInTheDocument();
  });
...
describe("Terminal - system locking", () => {
  ...
  it("ignores submissions while locked", async () => {
    ...
    // eslint-disable-next-line require-yield -- ジェネレーターである必要があるが、出力は意図的に空
    const slow: CommandAction = async function* () {
      await gated;
    };
    const user = userEvent.setup();
...
describe("Terminal - close button", () => {
  ...
  it("hides the close button when showCloseButton is false", () => {
    render(
      <Terminal commands={{}} onClose={() => {}} showCloseButton={false} />,
    );
    expect(
